                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <div class="row align-items-center">
                                            ...
                                            <div v-if="$userPermissions.includes('articles-create')==true"
                                                 class="col-auto"><a
                                    ...
                                    <div
                                        ...
                                        <div v-if="$userPermissions.includes('articles-read')==true">
                                            <vue-good-table
                                                :columns="columns"
                                                :rows="rows"
                                                :fixed-header="false"
                                                :line-numbers="true"
                                                @on-row-dblclick="function (params) {$inertia.visit(route('articles.edit', params.row.id));}"
                                                :search-options="{
                                                    enabled: true,
                                                    skipDiacritics: true,
                                                    placeholder: 'Поиск',
                                            ...
                                            >
                                                ...
                                                <template slot="table-row" slot-scope="props">
                                                    <span v-if="props.column.field == 'actions'">
                                                     ...
                                                     <a v-if="$userPermissions.includes('articles-read')==true"
                                                        :href="route('articles.show', props.row.id)"
                                                        title="Просмотр"
                                                        class="badge badge-info">
                                                         <i class="fe fe-eye"></i>
                                                     </a>

                                                     <a v-if="$userPermissions.includes('articles-update')==true"
                                                        :href="route('articles.edit', props.row.id)"
                                                        title="Редактировать" class="badge badge-warning">
                                                         <i class="fe fe-edit"></i>
                                                     </a>

                                                     <button v-if="$userPermissions.includes('articles-delete')==true"
                                                    ...
                                                    <span v-else-if="props.column.field == 'image'">
                                                        <div class="avatar avatar-xl">
                                                            <img :src="props.row.image.path || '/images/no_photo.png'"
    ...
    methods: {
        delete_record: function (id) {
            this.$swal({
                ...
                if (isConfirm === true) {
                    axios.delete(route("articles.destroy", id))
                        .then(
                            (response) => {
                                const index = this.articles.map(article => article.id).indexOf(id);
        ...
        saveColumns() {
            axios.put(route("users.update", this.$page.props.user.id), {component_columns: {article: this.columns}})
                .then(
                    (response) => {
