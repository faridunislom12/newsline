                                ...
                                <div class="card">
                                    <div class="card-header">
                                        <div class="row align-items-center">
                                            ...
                                            <div v-if="$userPermissions.includes('categories-create')==true"
                                                 class="col-auto"><a
                                    ...
                                    <div
                                        ...
                                        <div v-if="$userPermissions.includes('categories-read')==true">
                                            <vue-good-table
                                                :columns="columns"
                                                :rows="rows"
                                                :fixed-header="false"
                                                :line-numbers="true"
                                                @on-row-dblclick="function (params) {$inertia.visit(route('categories.edit', params.row.id));}"
                                                :search-options="{
                                                    enabled: true,
                                                    skipDiacritics: true,
                                                    placeholder: 'Поиск',
                                            ...
                                            >
                                                ...
                                                <template slot="table-row" slot-scope="props">
                                                    <span v-if="props.column.field == 'actions'">
                                                     ...
                                                     <a v-if="$userPermissions.includes('categories-read')==true"
                                                        :href="route('categories.show', props.row.id)"
                                                        title="Просмотр"
                                                        class="badge badge-info">
                                                         <i class="fe fe-eye"></i>
                                                     </a>

                                                     <a v-if="$userPermissions.includes('categories-update')==true"
                                                        :href="route('categories.edit', props.row.id)"
                                                        title="Редактировать" class="badge badge-warning">
                                                         <i class="fe fe-edit"></i>
                                                     </a>

                                                     <button v-if="$userPermissions.includes('categories-delete')==true"
...
export default {
    ...
    methods: {
        ...
        saveColumns() {
            axios.put(route("users.update", this.$page.props.user.id), {component_columns: {category: this.columns}})
                .then(
                    (response) => {
